...
Физически невозможные значения. Удалено 10 строк с Age > 80 или Employee Experience > 60.

Опечатка в названии колонки. Home Onwership → Home Ownership.

Устранение утечки данных (data leakage). Признак Previous Loan оказался детерминированным маркером: у всех заёмщиков с Previous Loan = Yes заявка была одобрена в 100% случаев (Mutual Information ≈ 0.191 — почти треть энтропии таргета). Такая связь нереалистична для банковского скоринга и, вероятнее всего, является артефактом генерации синтетических данных. Решение: все строки с Previous Loan = Yes удалены из выборки, сама колонка — тоже (после удаления она стала бы константой). Из-за этого размер датасета сократился до 22 135 строк, а баланс классов сместился с 78% / 22% до почти сбалансированных 55% / 45% — это ожидаемое следствие, а не ошибка. 
...
При Person Income = 0 вычисление Loan percentage даёт деление на ноль — на стороне API нет отдельной валидации этого случая.